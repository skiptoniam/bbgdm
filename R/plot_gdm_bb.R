...
# 
#     plot(bb.eta,Y[,1],xlab = "Linear Predictor",ylim=c(0,max(Y[,1])),#xlim=c(-5,5), 
#            ylab = paste0("Expectation of ",object$dism_metric," metric"), type = "n")
#     points(bb.eta,Y[,1], pch = 20, cex = 0.25, 
#              col = plot.colour)
#     y.pred <- Y[sample(pred_sample),]
#     y.pred <- y.pred[order(y.pred[,2], y.pred[,1]),]
#     overlayX.bb <- seq(from = min(bb.eta), to = max(bb.eta), 
...
#     lines(overlayX.bb, overlayY.bb*y.pred[,2], lwd = plot.linewidth,col=line.col)
    # mtext("Observed compositional dissimilarity vs. the linear predictor",outer = TRUE,cex = 1.5) 
}
